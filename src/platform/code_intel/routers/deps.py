...
@router.get("/graph")
async def get_import_graph(repo_id: str, language: str | None = None, min_imports: int = 1, data_dir: str = Query(default=None)):
    if data_dir is None:
...
async def get_file_imports(repo_id: str, path: str, data_dir: str = Query(default=None)):
    if data_dir is None:
        data_dir = str(DEFAULT_DATA_DIR)
    paths = _paths(repo_id, data_dir)
    api = registry.get_dep_api()
...
@router.get("/circular")
async def get_circular_deps(repo_id: str, data_dir: str = Query(default=None)):
    if data_dir is None:
        data_dir = str(DEFAULT_DATA_DIR)
    paths = _paths(repo_id, data_dir)
    api = registry.get_dep_api()
...
@router.get("/external")
async def get_external_packages(repo_id: str, data_dir: str = Query(default=None)):
    if data_dir is None:
        data_dir = str(DEFAULT_DATA_DIR)
    paths = _paths(repo_id, data_dir)
    api = registry.get_dep_api()
...
@router.get("/stats")
async def get_dependency_stats(repo_id: str, data_dir: str = Query(default=None)):
    if data_dir is None:
        data_dir = str(DEFAULT_DATA_DIR)
    paths = _paths(repo_id, data_dir)
    api = registry.get_dep_api()
