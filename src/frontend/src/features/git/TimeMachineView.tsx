...
import { getRepoTimeline, type TimelinePoint } from '../../api/git';

type TimePoint = TimelinePoint;
...
export function TimeMachineView({ repoId }: TimeMachineViewProps) {
    ...
    useEffect(() => {
        const loadData = async () => {
            setLoading(true);
            try {
                const data = await getRepoTimeline(repoId, {
                    points: 24,
                    granularity: 'monthly',
                });
                setTimeline(data.map(p => ({
                    date: p.date,
                    totalFiles: p.totalFiles,
                    totalCoupling: p.totalCoupling,
                    avgCoupling: p.avgCoupling,
                    hotspots: p.hotspots,
                    commits: p.commits,
